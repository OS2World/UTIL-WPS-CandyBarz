...
MRESULT EXPENTRY CandyBarZContainerProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    pfnOrigWndProc  = CBZShare.pContainerOrigWndProc; //get address of original wnd proc.
    ulDataOffset    = CBZShare.ulDataOffset;      //get offset to pCBZData

    return (pfnOrigWndProc(hwnd, msg, mp1, mp2));

#if 0
    /* WPS Container do not have a frame as parent */ 
    ...
    if ((pCBZData = WinQueryWindowPtr(hwndFrame, ulDataOffset)) == NULL)
      ...
      {
        if(!pCBZData->bContainerEnabled)
          return (pfnOrigWndProc(hwnd, msg, mp1, mp2));

        if(msg==CM_PAINTBACKGROUND){
          //pass messages to plugins for optional processing
          for (i = pCBZData->sContainerIndex; i < pCBZData->cbPlugins !!! ; i++)
            {
    ...
    switch (msg)
      ...
      case WM_PAINT:
        ...
        if (!CandyBarZContainerPaintProc(hwnd, pCBZData, 1) )
          {
