...
FILE* excOpenTraplogFile(VOID)
{
    ...
    sprintf(szFileName, "%c:\\%s", doshQueryBootDrive(), "CandyBar.log");
    file = fopen(szFileName, "a");

    if (file) {
        DosGetDateTime(&DT);
        fprintf(file, "\nCandyBarZ trap message -- Date: %02d/%02d/%04d, Time: %02d:%02d:%02d\n",
            DT.month, DT.day, DT.year,
            DT.hours, DT.minutes, DT.seconds);
        fprintf(file, "--------------------------------------------------------\n"
                      "\nAn internal error occurred within CandyBarz.\n");
